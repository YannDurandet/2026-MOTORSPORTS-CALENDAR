...
- Fonts: `--font-mono` (JetBrains Mono) for UI, labels, times; `--font-body` (Inter) for prose; `--font-title` (Orbitron) **only** for page titles (`h1`, series hero, newsletter hero) and calendar month headers. Files are self-hosted in `public/assets/fonts/` and precached in `public/sw.js` — bump `VERSION` there when fonts change.
